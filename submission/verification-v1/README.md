...
# Received laptop results

September 14: [the independently checked laptop evidence](../../docs/robotics/evidence/intel-final-legacy-v3/README.md) now verifies all six baseline skills with Intel CPU and Intel GPU.0 inference, including two fresh runs with Intel UHD OpenGL rendering. All source checkpoint hashes and the original kit manifest match. The i7-10850H is not Core Ultra Series 2/3; strict eligibility remains false. The newer late-mug live-vision option postdates this kit and requires its own verification. Original received ZIPs and personal summaries remain local; the reviewed derivative evidence is linked above.
