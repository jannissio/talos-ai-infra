...
class TableIK(ArmIK):
    """Exact grasp IK plus position-priority placement within task tilt limits.

    Real contact introduces a small held-object tilt. Requiring a mathematically
    exact body normal from this five-joint arm can reject usable placements.
    This accepts at most 8 degrees normal error and 0.25 mm point error; final
    physical placement/support/settling checks remain independent and unchanged.
    """
    soft_placement = False

    def solve(self, position, initial):
        if not self.soft_placement:return super().solve(position, initial)
        ...
        for _ in range(240):
            ...
            point = self.point(self.data); rotation = self.data.xmat[self.body].reshape(3, 3)
            axis = rotation[:, self.axis_index] if self.axis_local is None else rotation@self.axis_local
            position_error = position-point
            x_error = None if self.x_target is None else self.x_target-rotation[:, 0]
            if (np.linalg.norm(position_error) < .00025 and np.dot(axis, self.axis_target) > math.cos(math.radians(8))
                    and (x_error is None or np.linalg.norm(x_error) < .08)):
                return q
            mujoco.mj_jac(self.model, self.data, self.jp, self.jr, point, self.body)
            error = np.r_[position_error, .015*(self.axis_target-axis)]
            jac = np.vstack((self.jp[:, self.indices], -.015*skew(axis)@self.jr[:, self.indices]))
            if x_error is not None:
                error = np.r_[error, .015*x_error]
                jac = np.vstack((jac, -.015*skew(rotation[:, 0])@self.jr[:, self.indices]))
            dq = jac.T@np.linalg.solve(jac@jac.T+np.eye(len(error))*1e-6, error)
            # Preserve the descent direction. Clipping joints independently can
            # create a limit cycle for a held object offset from the fingertips.
            dq *= min(1., .08/max(float(np.max(np.abs(dq))), 1e-12))
            cost = float(error@error)
            for fraction in (1., .5, .25, .125, .0625):
                trial = np.clip(q+dq*fraction, self.lo, self.hi)
                self.data.qpos[self.indices] = trial
                mujoco.mj_kinematics(self.model, self.data)
                rr = self.data.xmat[self.body].reshape(3, 3)
                aa = rr[:, self.axis_index] if self.axis_local is None else rr@self.axis_local
                ee = np.r_[position-self.point(self.data), .015*(self.axis_target-aa)]
                if self.x_target is not None:ee = np.r_[ee, .015*(self.x_target-rr[:, 0])]
                if float(ee@ee) < cost:
                    q = trial; break
            else:break
        raise PlanningError('Placement position/orientation tolerance not solved for this grasp and arm.')
...
class TableTeacher(DinnerTask):
    ...
    def start(self, side='auto', object_id='bottle', target=None, excluded_grasps=(), **kwargs):
    ...
    def _precheck_transfer(self, grasp_q):
        ...
        if self.ik.axis_local is None and self.ik.axis_index == 2 and self.tube['id'] not in ('fork', 'spoon'):
            ...
            self.ik.x_target[2] = 0.; self.ik.x_target /= np.linalg.norm(self.ik.x_target)
        errors = []
        for height in (.065, .04, .10):
            try:
    ...
    def update(self, targets):
        ...
        try:
            ...
            if self.stage == 'approach' and done:
                ...
                if forces['base'] > .06 and abs(bottom-TABLE_Z) < .001 and xy_error < .01 and up > .98:
                    ...
                    gap = bottom-TABLE_Z
                    if count < 4 and 0. < gap < .012 and xy_error < .01 and up > .98:
