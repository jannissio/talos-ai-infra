...
**September 13:** the packaged v6 suite now includes six learned dinner skills plus both bottle-relay directions. Run `start-lab.ps1 -Learned -DinnerSuite models/dinner_suite/suite.json`, select **Learned dinner sequence**, and enter `set the table`. Frozen full-sequence evaluation is **8/10**; both relay directions are **5/5**. The original models and historical instructions below remain preserved. See [current architecture, reproduction and limitations](../docs/robotics/FINAL_ARCHITECTURE.md).
