...
DRAWER_TRAVEL = .095
OBJECTS = {
    "plate": {"label": "Blue-rim plate", "kind": "plate", "mass_kg": .065, "size_m": [.132, .132, .014], "color": "#719acb", "grasp_local_m": [0, -.061, .010], "grasp_width_m": .009},
    ...
    "fork": {"label": "Fork", "kind": "fork", "mass_kg": .012, "size_m": [.017, .110, .008], "color": "#cad5de", "grasp_local_m": [0, -.019, .004], "grasp_width_m": .009},
    "spoon": {"label": "Spoon", "kind": "spoon", "mass_kg": .014, "size_m": [.022, .110, .008], "color": "#cad5de", "grasp_local_m": [0, -.019, .004], "grasp_width_m": .009},
}
...
def make_object(world, object_id, position, yaw=0., mass_scale=1., friction_scale=1.):
    ...
    if kind == "plate":
        ...
        surface(body, f"{object_id}_handle", "box", (.0045, .034, .004), (0, -.019, .004), material="steel")
...
def add_drawer(world, opened):
    ...
    for sign in (-1, 1):
        surface(tray, f"drawer_side_{sign}", "box", (.003, .072, .026), (sign*.094, 0, .045), material="drawer_wood")
    surface(tray, "drawer_back", "box", (.094, .003, .026), (0, .069, .045), material="drawer_wood")
    surface(tray, "drawer_front", "box", (.108, .005, .035), (0, -.089, .045), material="drawer_wood")
    for x in (-.036, .036):
        surface(tray, f"drawer_handle_post_{x}", "box", (.004, .011, .004), (x, -.101, .052), material="steel")
    surface(tray, "drawer_handle", "capsule", (.005, .036), (0, -.113, .052), material="steel", quat=vec((math.sqrt(.5), 0, math.sqrt(.5), 0)))
    ...
    ET.SubElement(tray, "site", name="drawer_handle_grasp", pos="0 -.113 .052", size=".002", group="4")
...
def add_dinner_scene(root, seed, preset="task", drawer_open=False):
    ...
    targets = [
        {"id": "plate_place", "label": "Plate setting", "object_id": "plate", "position_m": [-.085, -.025, TABLE_Z], "radius_m": .074},
        {"id": "side_plate_place", "label": "Side plate setting", "object_id": "side_plate", "position_m": [.110, -.025, TABLE_Z], "radius_m": .060},
        {"id": "mug_place", "label": "Mug setting", "object_id": "mug", "position_m": [.185, .100, TABLE_Z], "radius_m": .033},
        {"id": "glass_place", "label": "Glass setting", "object_id": "glass", "position_m": [.075, .100, TABLE_Z], "radius_m": .033},
        {"id": "fork_place", "label": "Fork setting", "object_id": "fork", "position_m": [-.184, -.025, TABLE_Z], "radius_m": .012},
        {"id": "spoon_place", "label": "Spoon setting", "object_id": "spoon", "position_m": [.201, -.025, TABLE_Z], "radius_m": .014},
    ]
    for target in targets:
        x, y, z = target["position_m"]
        if target["object_id"] in ("spoon", "fork"):
            geom(world, target["id"], "box", (.014, .061, .00015), (x, y, z+.0006), rgba=".50 .51 .43 .55", contype="0", conaffinity="0")
        else:
    ...
    positions = {"plate": (-.275, .077), "side_plate": (-.278, .252),
                 "mug": (.272, .060), "glass": (.285, .245), "bottle": (.025, -.075)}
    records = []
    for object_id, spec in OBJECTS.items():
        ...
        if preset == "reference" and target:
            ...
            position = np.array([-.041 if object_id == "fork" else .041, .280-drawer["initial_open_m"], TABLE_Z+.030])
    ...
    return {"objects": records, "targets": targets, "drawer": drawer,
            ...
            "challenge": {"title": "Set the dinner table", "autonomy_available": False,
                          "instruction": "Open the drawer, place the plate and cutlery, then coordinate both arms to serve a drink.",
                          "next_skill": "Grasp the bottle neck, lift, and place it upright."}}
