...
**Latest status:** no complete jointly randomized seven-item table and no shared learned policy fit has passed. [V25/V27](evidence/shared-table-followup-v25-v27.json) are terminal: 267 physical attempts, 987,835 frames and 28 passing primitives, with 17 accepted continuous actions. V25 finishes only glass after exhausting 14 accepted actions; V27 finishes glass and spoon, then exhausts current candidates. [V26](evidence/shared-table-followup-v26.json) finishes only bottle after 72 attempts/216,579 frames. These separate item successes cannot be added into a complete workflow. The [V28 continuation experiment](experiments/verified-relay-continuation-development-v1.json) checks both physical legs before admitting a speculative buffer in the unchanged exposed V25 scene. The earlier frozen checkpoint below remains unchanged in scope.
...
The common integration now has a terminal [V24 result](evidence/shared-table-followup-v24.json): 75 physical attempts, 266,138 frames, eight passing primitives and five accepted continuous-scene actions. Inverted-bottle righting/relay and the glass buffer/horizontal regrasp finish both items at 0.612/0.361 mm. A sideways mug buffer passes too. The remaining five final settings fail; no complete table passes. Every motor replay is exact. V25 later tested another exposed joint scene; its terminal result appears at the top of this document.

Some original samples place an item outside all direct arm contact. The [side-plate reach evidence](evidence/sideplate-pregrasp-development-v1.json) preserves failed IK searches separately from a compiled-geometry separating proof. A [new candidate protocol](experiments/joint-direct-contact-development-v1.json) keeps the original sampler, all seven items and orientation families, retaining every rejected scene and ordinary settling trace before selecting the first scene without this necessary-condition failure. Its original 80-scene evidence and manipulation failures remain unchanged; no indirect-rearrangement or universal feasibility claim follows.

A shared visual policy still needs successful varied demonstrations and a fit. Handover remains September 14 at 18:00 CEST; the official deadline is September 16 at 20:30 CEST. GitHub and Hugging Face remain private, final Submit remains the user’s action, and all 41 protected published artifacts retain their original hashes.
