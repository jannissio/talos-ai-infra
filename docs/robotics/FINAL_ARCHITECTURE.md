...
The [production regressions](evidence/visual-mug-deployment-v1/README.md) verify both command entry points and both exposed seed-42 presets. A separate passive preview thread fixes hosted black frames without changing any recorded state, target or progress array. That profile checkpoint passes 73 application and 20 training tests. The option is selected locally at port 8770. The later [human voice and private cloud checks](evidence/human-voice-dinner-v1/README.md) both complete all six skills; the hosted CPU trial takes 287.79 wall seconds. New-option Intel execution remains separate.
...
The separate [shared-table pipeline](WHOLE_TABLE_PIPELINE.md) has calibrated RGB-D input and an adapted CLIPort runtime on the RTX 4070, but no trained Talos spatial policy or complete seven-item workflow. Its exact-state physical teacher, including terminal V28's four accepted actions and only one completed final setting, supplies development evidence rather than learned execution. Current full application checks pass 92 tests, followed by seven passing targeted checks of the corrected relay logic. [V7 submission text and slides](../../submission/final-v7/PROJECT.md) are saved in the draft with these limits explicit.
...
A later [stereo mug image-point observer](evidence/mug-keypoint-observer-v1/README.md) passes separately declared perception gates using a newly trained CNN and calibrated triangulation. It accepts every present case and rejects every absent case in 512 fresh posed observations (0.270 mm p95 / 0.422 mm maximum), and accepts all 144 exposed physical states (0.617 / 0.893 mm). The compact package reproduces all fresh images and predictions exactly. It has no corrective motor integration and does not change the execution path above. Live visual correction still requires an independently declared physical comparison.
