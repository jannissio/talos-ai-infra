...
- [ ] Complete [wider physical V1](experiments/bottle-wide-physical-v1.json), now frozen and running: eight fresh development scenes, then 32 fresh live/frozen/baseline comparisons plus six exposed regressions if its development gate passes. Use the same source rectangle and two demonstrated goals [0.10,-0.115] / [0.20,0.05]; the failed middle goal stays unsupported. Fresh scene seeds vary object starts, mass, friction and light. No fitting, camera, route or motor-threshold change belongs to this new task-level protocol. The older standalone synthetic perception failure remains disclosed.
