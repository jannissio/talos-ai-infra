...
def run(args):
    ...
    for seed,preset,variant in cases:
        ...
        output=root/args.split/name;log=ROOT/'.run/final-goal'/f'mug-visual-correction-v1-{args.split}-{name}.log'
    ...
    def one(job):
        ...
        with log.open('x',encoding='utf-8') as stream:
            result=subprocess.run([sys.executable,str(ROOT/'scripts/evaluate_mug_visual_correction.py'),
