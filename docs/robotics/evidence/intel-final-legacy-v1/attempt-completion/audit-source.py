...
def audit_run(raw, output, device, checkpoints, canonical_xml, layout):
    ...
    if ('i7-10850H' not in hardware['cpu'] or not hardware['intel_cpu'] or hardware['core_ultra_series'] is not None
            or hardware['strict_written_hardware_check'] or hardware['intel_opengl_renderer']
            or hardware['all_intel_cpu_and_graphics'] or verification['strict_hardware_and_physics_passed']):
    ...
    with np.load(output/'states.npz', allow_pickle=False) as states:
        ...
        for row in report['results']:
            ...
            if skill == 'drawer':
                ...
                error = float(np.linalg.norm(position[:2]-target['position_m'][:2])*1000)
...
def run(args):
    ...
    receipt = read(args.input/'evidence-manifest.json')
    ...
    with zipfile.ZipFile(original_kit) as kit:
        ...
        if kit_manifest != read(args.input/'kit-manifest.json'):
    ...
    for row in receipt['files']:
        ...
        if not source.resolve().is_relative_to(args.input.resolve()) or sha(source) != row['share_sha256'] or source.stat().st_size != row['bytes']:
    ...
    preflight = require_space(args.output, 64*1024**2)
    ...
    for source in sorted(args.input.rglob('*')):
        ...
        target = 'received/'+relative.replace('.run/', 'runs/', 1) if relative.startswith('.run/') else 'received/'+relative
        write(args.output/target, source.read_bytes())
        mapping[relative] = {'file': target, 'sha256': sha(source)}
    ...
    audit = {'schema': 'talos.intel-final-legacy-evidence.v1', 'received_files_checked': len(receipt['files']),
    ...
    lines = ['# Legacy Intel laptop verification', '',
        ...
        '| Inference | Six-skill result | Simulated seconds | Physics-loop wall seconds |',
        '| --- | --- | --- | --- |']
    for name, device in [('intel-final-laptop-cpu', 'CPU'), ('intel-final-laptop-gpu0', 'GPU.0 / Intel UHD')]:
        row = runs[name]
        lines.append(f"| {device} | 6/6 | {row['simulation_seconds']:.3f} | {row['physical_loop_wall_seconds']:.3f} |")
    lines += ['', '**Hardware eligibility remains unresolved.** The processor is not Core Ultra Series 2/3. OpenGL rendering used the NVIDIA Quadro T2000 Max-Q. Every strict hardware flag remains false. CPU/iGPU inference is verified; all-Intel rendering and a qualifying Core Ultra run are not.', '',
        'Each network benchmark uses FP32, 20 warmups and 300 measured synchronous calls; one output contains 20 action endpoints. These small-network timings exclude vision, rendering and physics. CPU medians range from 0.121 to 0.163 ms; Intel UHD medians range from 0.594 to 0.670 ms.', '',
        ...
        'All 87 transport-manifest files match; the original 318-file kit manifest and six source checkpoint hashes match. Both complete 4,955-frame traces are unchanged. Portable scenes load, every trace array is checked, and restored completion geometry agrees with the original reports. Continuous contact/hold evidence comes from the retained original runtime monitor; the 20 Hz saved trace cannot independently reconstruct every 200 Hz contact.', '',
