"""Freeze and compare complete browser-engine workflows for spoon-release-v1."""
import argparse
...
def budget(path, expected):
    ...
    folders = (BASE,ROOT/'training/spoon_release_v1',ROOT/'models/spoon_release_v1',ROOT/'docs/robotics/evidence/spoon-release-v1')
...
def inputs():
    ...
    replay = ROOT/'training/spoon_release_v1/input-replay.json'
    ...
    if (sha(ROOT/protocol['baseline_spoon']/'primitive.safetensors') != protocol['baseline_spoon_sha256']
            or training['warm_start_sha256'] != protocol['baseline_spoon_sha256']):
    ...
    if set(candidate) != set(baseline):
        raise ValueError('Only the existing spoon model may be replaced.')
    for skill in baseline:
        if skill != 'spoon' and (suites['baseline'].parent/baseline[skill]).resolve() != (suites['candidate'].parent/candidate[skill]).resolve():
            raise ValueError('Another model changed: '+skill)
    model = (suites['candidate'].parent/candidate['spoon']).resolve()
...
def aggregate(rows,split):
    ...
    spoons = {c:{p:sum(r.get('spoon_status')=='succeeded' for r in rows if r['controller']==c and r['preset']==p) for p in PRESETS} for c in CONTROLLERS}
    ...
    if split == 'development':
        gate = complete and pairs_valid and all(counts['candidate'][p] >= 5 and counts['candidate'][p] >= counts['baseline'][p]
                                and spoons['candidate'][p] >= spoons['baseline'][p] for p in PRESETS)
    else:
    ...
    return {'complete':complete,'attempted':len(rows),'planned':len(expected),'passed_workflows':counts,
            'passed_spoon_attempts':spoons,'paired_initial_states_match':pairs_valid,'pairs':paired,
...
def freeze(args):
    ...
    result = {'schema':protocol['schema'],'split':args.split,'inputs':current,
              ...
              'scope':'Paired actual language/RGB-engine workflows; changed spoon only, unchanged physical thresholds.'}
...
def batch(args):
    ...
    def one(seed,preset,controller):
        ...
        command = [sys.executable,str(Path(__file__).resolve()),'trial','--freeze',str(args.freeze.resolve()),
        ...
        report = read(path)
        spoon = next((r for r in report.get('task',{}).get('results',[]) if r['skill']=='spoon'),None)
        ...
        return {'seed':seed,'preset':preset,'controller':controller,'passed':passed,'status':report['status'],
                'spoon_status':None if spoon is None else spoon['status'],
                'spoon_placement_mm':None if spoon is None else spoon['metrics'].get('placement_error_mm'),
