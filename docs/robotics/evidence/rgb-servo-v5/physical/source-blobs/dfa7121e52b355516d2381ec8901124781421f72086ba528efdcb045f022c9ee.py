...
def run(args):
    ...
    packaged=json.loads((args.model/'experiment.json').read_text())
    for name in ('observer_sha256','motor_sha256','openvino_artifact_sha256'):
        if report[name]!=packaged[name]:raise ValueError('Packaged model artifact mismatch: '+name)
    ...
    try:
        for tick in range(40001):
            ...
            if monitor and monitor.succeeded:
                ...
                controller=None;monitor=None;target=data.qpos[:12].copy();waiting_since=now
            ...
            elif route and controller is None and now-waiting_since>2.:message='No fresh RGB observation for the next relay leg.';break
            data.xfrc_applied[:]=0.
