...
def fingerprint(p,protocol):
    original=ROOT/p['unchanged_control_protocol']
    if sha(original)!=p['unchanged_control_protocol_sha256']:raise ValueError('The original controller declaration changed.')
    prior=read(original)
    for key in ('control','observer','observer_manifest_sha256','motor','motor_manifest_sha256','baseline_suite','baseline_suite_sha256','interface'):
        if p[key]!=prior[key]:raise ValueError('V2 must preserve controller settings, models and interface: '+key)
    for key in ('prior_physical_audit','renderer_diagnostic'):
        if sha(ROOT/p[key])!=p[key+'_sha256']:raise ValueError('Preserved prerequisite changed: '+key)
    diagnosis=read(ROOT/p['renderer_diagnostic'])
    if not diagnosis['close_before_create_matches_single_rgb'] or diagnosis['integration_steps']!=0:
        raise ValueError('Passing no-step renderer diagnosis required.')
    if read(ROOT/p['prior_physical_audit'])['gate_passed']:raise ValueError('V1 must remain stopped.')
    ...
    sources += [Path(__file__),ROOT/'scripts/probe_mug_correction_interface.py',ROOT/'scripts/run_mug_visual_correction_v2.py',
...
def trial(p,args):
    ...
    try:
        ...
        if args.variant!='baseline':
            ...
            replacement.plan=plan;engine.task=replacement
