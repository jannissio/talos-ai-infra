...
from scripts.evaluate_refined_bottle_physics import execute_trial, integration_path, checked_integration, bottle_destination
...
def declare(path):
    ...
    p = {'schema': 'talos.bottle-refinement-exposed-physics.v2', 'declared_on': '2026-09-14',
        'raw_root': '.run/bottle-refinement-exposed-physics-v2', 'model_package': parent['model_package'],
        'training_package': parent['training_package'], 'evidence_package': 'docs/robotics/evidence/bottle-refinement-exposed-physics-v2',
        ...
        'preceding_attempt': {'protocol': 'docs/robotics/experiments/bottle-refinement-exposed-physics-v1.json',
            'result': 'Both anchors stopped before observations or control because the task-preset bottle destination is implicit. StopIteration, zero recorded control frames. All files remain retained.',
            'repair': 'Read the original declared layout default: bottle initial XY plus [0.07,-0.04] when no explicit target exists. This is the existing task destination, not a new controller target or physical threshold.'},
