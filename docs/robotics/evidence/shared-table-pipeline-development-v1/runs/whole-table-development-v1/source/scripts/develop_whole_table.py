...
def run(folder, seed, maximum_actions, only_item=None):
    if folder.exists():raise FileExistsError('Preserve every development attempt.')
    preflight = require_space(folder, 4*1024**3); folder.mkdir(parents=True)
    ...
    try:
        ...
        for action_index in range(maximum_actions):
            ...
            for attempt_index, (name, target, role) in enumerate(options):
                task = TableTeacher(model, data, layout); task.start(object_id=name, target=target)
                task.update(targets)
                if task.active:
                    selected = task, role; break
                failure = {'action_index': action_index, 'attempt_index': attempt_index,
                           'item': name, 'role': role, 'message': task.message, 'search': task.search_log}
                report['planning_failures'].append(failure)
                put(folder/f'planning-{action_index:02d}-{attempt_index:03d}.json', failure)
            ...
            if selected is None:
                ...
                break
            task, role = selected
            result = execute(model, data, layout, task, targets, folder/f'action-{action_index:02d}-{task.tube["id"]}', observer)
            result['role'] = role; report['actions'].append(result)
            if not result['demonstration_eligible']:
                report['stop_reason'] = 'Physical action failed; state and trace retained. Physical recovery remains unimplemented.'
                break
            if role == 'final_setting':remaining.remove(task.tube['id'])
            if only_item:break
            if not remaining:break
        report['remaining'] = remaining
        report['whole_table_complete'] = not remaining and only_item is None
        ...
        print({k: v for k, v in report.items() if k not in ('actions', 'planning_failures', 'final_geometry')}, flush=True)
...
if __name__ == '__main__':
    ...
    args = parser.parse_args(); run(args.output, args.seed, args.maximum_actions, args.only_item)
