...
def audit_run(raw, output, device, checkpoints, canonical_xml, layout, intel_renderer=False):
    ...
    with np.load(output/'states.npz', allow_pickle=False) as states:
        ...
        for row in report['results']:
            total += row['elapsed_s']
            eligible = np.flatnonzero((states['stage'] == row['skill']) & (states['time'] <= total+1e-6))
            if len(eligible) == 0:
                raise ValueError('No retained frame belongs to the completed skill.')
            index = int(eligible[-1])
            if abs(float(states['time'][index])-total) > .051:
            ...
            end_geometry.append({'skill': skill, 'state_index': index, 'state_time': float(data.time),
                'seconds_before_reported_completion': total-float(data.time),
    ...
    return {'device': device, 'physical_passed': True, 'trace_frames': frames,
        ...
        'trace_audit_scope': 'All arrays, stages, chronology and original bytes checked; the final sampled frame of each completed skill (within one 20 Hz interval) is restored with mj_forward. No integration step or fresh laptop run. Continuous contact/hold claims are from the retained original 200 Hz monitor; the saved trace samples at 20 Hz.'}
