...
Use unused output and console-log paths and keep the 10 GiB reserve. The subsequent [single 18,000-update fit and physical comparison](../../docs/robotics/evidence/mug-release-v1/README.md) are complete: training/export pass, but the candidate regresses to 4/6 full workflows per preset against the baseline's 6/6. It is not promoted; final scenes remain unexposed. Training replays do not replace candidate physical evidence. The selected dinner suite, prior spoon candidate, hosted demo and submission materials remain unchanged.
